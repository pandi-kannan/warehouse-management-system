                ...
                <div style={styles.formCard}>
                    ...
                    <div style={styles.formGrid}>
                        <div style={styles.fieldGroup}>
                            ...
                            <select style={styles.input} value={receiveForm.productId} onChange={e => setReceiveForm({ ...receiveForm, productId: e.target.value })}>
                        ...
                        <div style={styles.fieldGroup}>
                            ...
                            <input style={styles.input} type="number" placeholder="Enter quantity" value={receiveForm.quantity} onChange={e => setReceiveForm({ ...receiveForm, quantity: e.target.value })} />
                        </div>
                        <div style={styles.fieldGroup}>
                            <label style={styles.label}>Storage Bin (uses first bin if not selected)</label>
                            <select style={styles.input} value={receiveForm.binId} onChange={e => setReceiveForm({ ...receiveForm, binId: e.target.value })}>
                                <option value="">Auto-select bin</option>
                                {bins.map(b => (
                    ...
                    <button style={{ ...styles.btnPrimary, background: '#10b981' }} onClick={handleReceive}>
                        📥 Receive Stock
                    </button>
                </div>
            )}
            ...
            {activeTab === 'dispatch' && (
                <div style={styles.formCard}>
                    ...
                    <div style={styles.formRow}>
                        <select style={styles.input} value={dispatchForm.productId} onChange={e => setDispatchForm({ ...dispatchForm, productId: e.target.value })}>
                            <option value="">Select Product</option>
                            {products.map(p => <option key={p.id} value={p.id}>{p.name} ({p.sku})</option>)}
                        </select>
                        <input style={styles.input} type="number" placeholder="Quantity" value={dispatchForm.quantity} onChange={e => setDispatchForm({ ...dispatchForm, quantity: e.target.value })} />
            ...
            {activeTab === 'all' && (
                loading ? <div style={styles.loading}>Loading inventory...</div> : (
                    <div style={styles.table}>
                        ...
                        {inventory.length === 0 && <div style={styles.empty}>No inventory records.</div>}
                        {inventory.map(item => (
                            <div key={item.id} style={styles.tableRow}>
                                <span style={styles.productName}>{getProductName(item)}</span>
                                <span style={styles.badge}>{item.product?.sku || '—'}</span>
                                <span style={styles.cell}>{getBinCode(item)}</span>
